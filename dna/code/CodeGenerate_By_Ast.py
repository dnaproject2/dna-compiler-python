...
from dna.util import Digest
from dna.code.astvmtoken import AstVMTokenizer
from dna.interop import VMOp
from binascii import a2b_hex
from dna import __version__
from dna.code.StaticAppCall import RegisterAppCall, NotifyAction

# Global arg set.
DNA_SC_FRAMEWORK = 'dna.interop.'
DNA_SC_FRAMEWORK_boa = 'boa.interop.'
DNA_BUILTINS_M = 'dna.builtins'
DNA_BUILTINS_M_boa = 'boa.builtins'
    ...
    def generic_visit(self, node):
        if hasattr(node, 'lineno') and node.lineno == 1:
            if type(node).__name__ == 'Assign' and len(node.targets) == 1 and type(node.targets[0]).__name__ == 'Name' and type(node.value).__name__ == 'Str':
                self.DnaCversion = node.value.s
                if self.DnaCversion != __version__ or node.targets[0].id != 'DnaCversion':
                    Print_Warning_global(self.codegencontext.main_file_path, node, "Place 'DnaCversion = '%s'' at 1st line of SmartContract." % (__version__))
            else:
                Print_Warning_global(self.codegencontext.main_file_path, node, "Place 'DnaCversion = '%s'' at 1st line of SmartContract" % (__version__))
            return
...
class Visitor_Of_FuncDecl(ast.NodeVisitor):
    def __init__(self, codegencontext, visited_module, list_func_imported):
        ...
        if (DNA_SC_FRAMEWORK in self.visited_module) or (DNA_SC_FRAMEWORK_boa in self.visited_module):
            self.isyscall_module = True
        elif (self.visited_module == DNA_BUILTINS_M) or (self.visited_module == DNA_BUILTINS_M_boa):
    ...
    def visit_ImportFrom(self, node):
        ...
        for alias in node.names:
            ...
            if alias.name == 'range' and (node.module == DNA_BUILTINS_M or node.module == DNA_BUILTINS_M_boa):
...
class Visitor_Of_FunCodeGen(ast.NodeVisitor):
    ...
    def visit_For(self, node):
        ...
        target_position = self.func_desc.Get_LocalStackPosition(node.target.id, node.target) # here only support Name taget. so here just ref it
    ...
    def visit_Call(self, node):
        ...
        if not (func_desc.is_builtin or func_desc.isyscall):
            ...
            if action_reset_the_syscall_name:
                ...
                syscall_name = syscall_name.replace(DNA_SC_FRAMEWORK_boa, '').encode('utf-8')
            ...
            syscall_print = sys_name.replace(DNA_SC_FRAMEWORK, '')
            syscall_print = syscall_print.replace(DNA_SC_FRAMEWORK_boa, '')
...
class CodeGenContext:
    ...
    def Generate_Debug_Json(self, output_path, data):
        ...
        JsonMap['compiler'] = {'name': 'dna-Python-Compile', 'version': __version__}
        files = {}
        JsonMap['files'] = files
        prev_lineno = -1
        asmap = []
    ...
    def NewFunc(self, node, isyscall, filepath, module_name, is_builtin):
        ...
        if name in self.funcscope.keys():
            ...
            # for the compatibility of boa.xxx.yyy and dna.xxx.yyy
